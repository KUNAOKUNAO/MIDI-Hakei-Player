...
class SimpleMIDIPlayer200Design:
    ...
    def _finalize_ended_process(self):
        """If a previous fluidsynth proc is set but already ended, clean it up and reset UI."""
        if self.proc is not None and self.proc.poll() is not None:
            # Already dead; clear and reset state
            self.proc = None
            self.running = False
            self.paused = False
            self._apply_state("stopped")
...
def start(self):
        # Make sure any defunct process is cleaned up
        ...
        try:
            ...
            # process poller: check periodically without blocking UI
